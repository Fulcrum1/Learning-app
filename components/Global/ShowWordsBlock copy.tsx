...
type WordsResponse = {
  words: WordItem[];
  knowWords: { wordId: string }[];
  learnWords: { wordId: string }[];
  unknownWords: { wordId: string }[];
};
...
export default function ShowWords({
  ...
  const fetchWords = async () => {
    ...
    setError(null);
    try {
      const response = await apiRequest.get(`${BACKEND_URL}/api/words`);
      if (!response.ok) {
  ...
  const getWordsByIds = (ids: { wordId: string }[]) => {
    return words.words.filter((item) =>
      ids.some((idObj) => idObj.wordId === item.id)
    );
  };
  ...
  const getFilteredWords = () => {
    switch (selectedCategory) {
      case "mastered":
        return getWordsByIds(words.knowWords);
      case "learning":
        return getWordsByIds(words.learnWords);
      case "toReview":
        return getWordsByIds(words.unknownWords);
      default:
        return words.words;
    }
  };
          ...
          <div className="border-b">
            <div className="flex gap-2 flex-wrap pb-4 ml-4">
              ...
              >
                ...
                Mastered ({words.knowWords.length})
              </Button>
              <Button
                variant={
              ...
              >
                ...
                En cours ({words.learnWords.length})
              </Button>
              <Button
                variant={
              ...
              >
                ...
                To review ({words.unknownWords.length})
              </Button>
            </div>
          </div>
