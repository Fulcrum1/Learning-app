...
export default function CardsComponent({
  ...
  const progress = useMemo(
    () => ((currentIndex + 1) / word.length) * 100,
  ...
  const handleWordKnow = useCallback(() => {
  ...
  }, [currentCard, handleUpdateProgress, animateCardOut, word.length]);

  const handleWordUnknown = useCallback(() => {
    if (!currentCard?.id) return;
  ...
  const handlePrevious = useCallback(async () => {
    if (currentIndex > 0) {
      ...
      await apiRequest.put(`${BACKEND_URL}/api/card/rollback-progress-card`, {
        listId: id,
        wordId: word[newIndex]?.id,
      });
    }
  }, [currentIndex, id, word]);
  ...
  }, [endOfList, handleTurn, handleWordKnow, handleWordUnknown]);
